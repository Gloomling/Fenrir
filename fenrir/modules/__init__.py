...
try:
    from fenrir.port_scanner import PortScanner, parse_ports, DEFAULT_PORTS, WEB_PORTS, SSH_PORT
except ImportError as e:
...
try:
    from fenrir.vulnerability_scanner import VulnerabilityScanner
except ImportError as e:
    log.warning(f"VulnerabilityScanner could not be loaded: {e}")
    VulnerabilityScanner = None  # type: ignore[assignment,misc]

try:
    from fenrir.exploit_scanner import ExploitScanner
except ImportError as e:
...
try:
    from fenrir.password_sprayer import PasswordSprayer
except ImportError as e:
