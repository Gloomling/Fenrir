...
""",
    ...
    # ── Network scan (multi-host) ──────────────────────────────────────────────
    net = parser.add_argument_group("Network / Multi-Host Scan")
    net.add_argument(
        "--net-scan",
        metavar="RANGE",
        default=None,
        help=(
            "Discover and deeply assess all hosts in a range.\n"
            "Accepts CIDR, hyphen range, comma-separated IPs, or mixed:\n"
            "  192.168.1.0/24\n"
            "  10.0.0.1-50\n"
            "  192.168.1.5,192.168.1.10\n"
            "  192.168.1.0/24,10.0.0.1\n"
            "Runs OS fingerprinting, CVE lookup, and exploit matching per host.\n"
            "Does not require a positional TARGET."
        ),
    )
    net.add_argument(
        "--net-modules",
        metavar="MODULES",
        default="vuln,os,exploit,iot,mobile",
        help=(
            "Comma-separated modules for network scan. Default: vuln,os,exploit,iot,mobile\n"
            "Options: vuln, os, web, exploit, iot, mobile"
        ),
    )
    net.add_argument(
        "--net-concurrency",
        ...
        default=5,
        metavar="N",
        help="Max simultaneous host assessments. Default: 5.",
    )
    net.add_argument(
        "--net-no-mobile",
        action="store_true",
        help="Skip AndroidScanner on mobile-classified hosts.",
    )
    net.add_argument(
        "--net-no-iot",
        action="store_true",
        help="Skip IoT default credential checks.",
    )
...
async def _run_scans(args: argparse.Namespace) -> int:
    ...
    # ── Network scan (multi-host) — doesn't need positional TARGET ───────────
    if getattr(args, "net_scan", None):
        if NetworkScanner is None:
            log.error("NetworkScanner not available. Check dependencies.")
            return 1
        net_range = args.net_scan
        net_mods  = set(m.strip() for m in args.net_modules.split(",") if m.strip())
        log.info(f"Network scan: {net_range}  modules: {net_mods}")

        report = ReportManager(str(output_dir),
                               f"network_{net_range.replace('/', '_').replace(',', '_')}")
        summary = await NetworkScanner().run(
            net_range,
            modules=net_mods,
            max_concurrent_hosts=args.net_concurrency,
            port_timeout=getattr(args, "port_timeout", 1.5),
            include_mobile=not args.net_no_mobile,
            include_iot=not args.net_no_iot,
            report=report,
        )
        report.finalize()
        if summary:
            print(f"\n{'═'*58}")
            print(f"  Network Scan Complete — {net_range}")
            print(f"  Hosts scanned : {summary.get('total_hosts', 0)}")
            print(f"  Hosts live    : {summary.get('live_hosts', 0)}")
            total_cves = summary.get('total_cves', 0)
            crit_hosts = summary.get('critical_hosts', 0)
            print(f"  Total CVEs    : {total_cves}")
            print(f"  Critical hosts: {crit_hosts}")
            for path in (report.txt_path, report.json_path):
                if path:
                    print(f"  Report: {path}")
            print(f"{'═'*58}\n")
        return 0
